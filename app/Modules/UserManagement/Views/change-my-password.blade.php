@extends('layouts.master') @section('title','Update Password')
...
@section('content')
	...
	<section class="content">
        ...
        <div class="box">
            ...
            <div class="box-body">
                <form role="form" action="{{url('user/changeMyPassword')}}" class="form-validation" method="post" onsubmit="return checkPassword();">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="form-group">
                                <label class="control-label required">Current Password</label>
                                <input type="password" class="form-control @if($errors->has('password')) error @endif"  id="password" name="password" placeholder="Current Password">
                                @if($errors->has('password'))
                                    <label id="label-error" style="color: red" class="error" for="password">{{$errors->first('password')}}</label>
                                @endif
                                <input type="hidden" name="user_id" id="user_id" value="{{ $id }}">
                                <input type="hidden" name="password_confirmation" id="password_confirmation" value="{{ $pwd }}">
                            </div>
                        </div>
                    </div>    
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="form-group">
                                ...
                                <input type="password" class="form-control @if($errors->has('new_password')) error @endif"  id="new_password" name="new_password" placeholder="Password">
                                @if($errors->has('new_password'))
                                    <label id="label-error" style="color: red" class="error" for="new_password">{{$errors->first('new_password')}}</label>
                                @endif
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
